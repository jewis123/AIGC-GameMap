...
{
    public class DrawToImgIPSegProcessor:BaseProcessor
    {
        private readonly string _workflowPath;

        public DrawToImgIPSegProcessor(ref ComfyUIClient comfyClient):base(ref comfyClient)
        {
        ...
        public async Task<string?> Process(string prompt, string drawImagePath, string refImgPath, int[] pixcels, IProgress<int> progress)
        {
            try
            {
                ...
                // 轮询等待结果
                string resultImagePath = await _comfyClient.PollForResult(promptId, "25", pixcels[0] * pixcels[1] / (512*512)); // 25是SaveImage节点ID
