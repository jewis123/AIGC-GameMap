...
namespace MapGenerator.Request.ComfyUI
{
    public class DrawToImgProcessor : BaseProcessor
    {
        ...
        public async Task<string?> Process(string prompt, string imagePath, int[] pixcelSize, IProgress<int> progress)
        {
            try
            {
                // 先取消当前正在执行的任务，避免排队
                ...
                // 轮询等待结果
                string resultImagePath = await _comfyClient.PollForResult(promptId, "34"); // 34是SaveImage节点ID
        ...
        private async Task<Dictionary<string, object>?> PrepareWorkflow(string prompt, string uploadedImageName, int[] pixcelSize)
        {
            try
            {
                ...
                var node37 = JsonSerializer.Deserialize<Dictionary<string, object>>(
                    workflow["37"].GetRawText());

                if (node37 != null && node37.ContainsKey("inputs"))
                {
                    var inputs = JsonSerializer.Deserialize<Dictionary<string, object>>(
                        ((JsonElement)node37["inputs"]).GetRawText());

                    if (inputs != null)
                    {
                        inputs["text"] = prompt;
                        node37["inputs"] = inputs;
                        modifiedWorkflow["37"] = node37;
                    }
                }
