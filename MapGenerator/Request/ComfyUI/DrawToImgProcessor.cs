...
{
    public class DrawToImgProcessor
    {
        private readonly ComfyUIClient _comfyClient;
        private readonly string _workflowPath;

        public DrawToImgProcessor(ref ComfyUIClient comfyClient)
        {
        ...
        public async Task<string?> ProcessDrawToImage(string prompt, string imagePath, int[] pixcels)
        {
            try
            {
                // 先取消当前正在执行的任务，避免排队
